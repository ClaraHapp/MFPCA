...
#' @param penMat A list of two roughness penalization matrices, one for each image
#'   direction (v & w). No smoothing along observations.
#' @param alphaVal A list of length 2, containing a vector of smoothing parameter values for each image direction (v & w).
#' @param verbose Logical. If \code{TRUE} computational details are given on the
#'   standard output during calculation of the FPCA
#'  @param tol A numeric value, given the tolerance for relative and absolute values in the algorithm. Defaults to \code{1e-4}.
#'   
